  ...
  return (
    <div ref={dragDropContext.componentsRef} className='bg-muted/40 flex h-full flex-col gap-2 p-5'>
      {dragDropContext.components.map((dragDropComponent) => (
        <React.Fragment key={dragDropComponent.id}>
          {'children' in dragDropComponent && (
            <CompositeComponent dragDropComponent={dragDropComponent} />
          )}
          {!('children' in dragDropComponent) && (
            <LeafComponent dragDropComponent={dragDropComponent} />
          )}
        </React.Fragment>
      ))}
